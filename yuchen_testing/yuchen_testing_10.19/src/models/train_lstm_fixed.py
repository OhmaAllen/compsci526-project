"""
LSTM Training Script - Independent with Fixes
"""
import torch
...
MODELS_DIR = RESULTS_DIR / 'models' / 'lstm'
...
log("Checking class distribution...")
...
log(f"✓ Positive rate: {pos_rate:.3f} ({pos_rate*100:.1f}%)")
log("")
...
# Create model with better config
log("Creating LSTM model...")
config = {
    'lstm_hidden_size': 64,  # Smaller to prevent overfitting
    'lstm_num_layers': 1,    # Single layer
    'lstm_dropout': 0.0,     # No dropout in LSTM
    'fc_hidden_size': 32,    # Smaller FC
    'fc_dropout': 0.2        # Light dropout
}
...
pos_weight = torch.tensor([1.0 / pos_rate - 1.0])  # Balance classes
criterion = nn.BCEWithLogitsLoss(pos_weight=pos_weight)
log(f"Using pos_weight: {pos_weight.item():.3f} to handle class imbalance")
...
with open(MODELS_DIR / 'lstm_results.json', 'w') as f:
    json.dump(results, f, indent=2)

log("="*70)
log("✓ TRAINING COMPLETE!")
log(f"✓ Results saved to: {MODELS_DIR / 'lstm_results.json'}")
log("="*70)
